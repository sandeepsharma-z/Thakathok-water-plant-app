...
export function DeliveryForm({bookingId,balance,cans,locale}:{bookingId:string;balance:number;cans:number;locale:Locale}){
  ...
  const[returnedCansInput,setReturnedCansInput]=useState("");
  const cashCollected=Number(cashInput)||0;
  const returnedCans=Number(returnedCansInput)||0;
  const remainingBalance=Math.max(0,balance-cashCollected);
  const pendingCans=Math.max(0,cans-returnedCans);
  ...
  return <form action={async(form)=>{if(canvas.current)form.set("customer_signature",canvas.current.toDataURL("image/png"));await action(form);}} className="mt-4 space-y-3 border-t border-blue-100 pt-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      ...
      <label className="rounded-2xl bg-blue-50/60 p-3 text-xs font-bold"><span className="flex gap-2"><PackageCheck className="h-4 w-4 text-blue-600"/>{t(locale,"Empty Cans Returned")}</span><input name="empty_cans_returned" type="number" inputMode="numeric" min="0" max={cans} value={returnedCansInput} placeholder="0" onChange={(event)=>{const raw=event.target.value;if(raw===""){setReturnedCansInput("");return;}setReturnedCansInput(raw.replace(/^0+(?=\d)/,""));}} className="mt-2 h-11 w-full rounded-xl border border-blue-100 bg-white px-3 outline-none"/><small className="font-normal text-slate-500">{t(locale,"Cans at Customer")}: {pendingCans}</small></label>
    </div>
    ...
    <div className="rounded-2xl border border-blue-100 p-3"><p className="mb-2 flex gap-2 text-xs font-bold"><PenLine className="h-4 w-4 text-blue-600"/>{t(locale,"Customer Signature (optional)")}</p><canvas ref={canvas} onPointerDown={start} onPointerMove={move} onPointerUp={()=>setDrawing(false)} onPointerCancel={()=>setDrawing(false)} className="h-28 w-full touch-none rounded-xl bg-blue-50/60"/></div>
    <textarea name="notes" placeholder={t(locale,"Delivery notes (optional)")} className="min-h-20 w-full rounded-2xl border border-blue-100 p-3 text-xs outline-none"/>
    {state.error||state.ok?<p className={`rounded-xl px-3 py-2 text-xs font-bold ${state.error?"bg-red-50 text-red-600":"bg-emerald-50 text-emerald-700"}`}>{t(locale,state.error??state.ok??"")}</p>:null}
    <button disabled={pending} className="h-12 w-full rounded-2xl bg-gradient-to-r from-[#075bd8] to-[#1b9cf0] text-sm font-black text-white shadow-lg disabled:opacity-60">{t(locale,pending?"Saving...":"MARK DELIVERED & SAVE")}</button>
  </form>;
}
