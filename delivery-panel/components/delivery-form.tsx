...
export function DeliveryForm({bookingId,balance,cans,locale}:{bookingId:string;balance:number;cans:number;locale:Locale}){
  ...
  return <form action={async(form)=>{if(canvas.current)form.set("customer_signature",canvas.current.toDataURL("image/png"));await action(form);}} className="mt-4 space-y-3 border-t border-blue-100 pt-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      ...
      <label className="rounded-2xl bg-blue-50/60 p-3 text-xs font-bold"><span className="flex gap-2"><PackageCheck className="h-4 w-4 text-blue-600"/>{t(locale,"Empty Cans Returned")}</span><input name="empty_cans_returned" type="number" inputMode="numeric" min="0" max={cans} value={returnedCansInput} placeholder="0" onChange={(event)=>{const raw=event.target.value;if(raw===""){setReturnedCansInput("");return;}setReturnedCansInput(raw.replace(/^0+(?=\d)/,""));}} className="mt-2 h-11 w-full rounded-xl border border-blue-100 bg-white px-3 outline-none"/><small className="font-normal text-slate-500">{t(locale,"Pending Cans")}: {pendingCans}</small></label>
    </div>
