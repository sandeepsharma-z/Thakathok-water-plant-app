...
export default async function ReportsPage({
  ...
  const [{ data: bookings }, { data: expenses }, { data: inventory }, { data: collections }] =
    await Promise.all([
      ...
      db.from("booking_collections").select("amount,collected_at").gte("collected_at",start).lt("collected_at",end),
    ]);
  const bs = bookings ?? [];
  const es = expenses ?? [];
  const active = bs.filter((booking) => booking.status !== "cancelled");
  const confirmed = active.filter((booking) => booking.status === "confirmed" || booking.status === "delivered");
  const revenue = confirmed.reduce((sum, booking) => sum + Number(booking.grand_total), 0);
  const collected = (collections ?? []).reduce((sum, collection) => sum + Number(collection.amount), 0);
  const dues = confirmed.reduce((sum, booking) => sum + Number(booking.balance), 0);
  const cans = confirmed.reduce((sum, booking) => sum + Number(booking.cans), 0);
  ...
  const summary = { bookings: confirmed.length, cans, revenue, collected, dues, expenses: expense, margin: revenue - expense };
  ...
  return (
    <>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatTile label="Confirmed bookings" value={confirmed.length} icon="clipboard" />
        ...
        <StatTile label="Payments collected" value={collected} icon="wallet" money />
